...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        loss, dS = softmax_loss(out2, y)
        loss += 0.5 * self.reg * (np.sum(W1*W1) + np.sum(W2*W2))
        
        dx, dW2, db2 = affine_backward(dS, cache2)
        grads['b2'] = db2
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for idx, hidden_dim in enumerate(hidden_dims):
            if idx == 0:
                ...
                self.params['W' + str(idx + 1)] = np.random.normal(scale=weight_scale, size=(hidden_dims[idx - 1], hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers - 1):
            ...
            if self.use_batchnorm:
                ...
                prev_out, cache[layer + 1] = batchnorm_relu_forward(prev_out, W, b, gamma, beta, bn_param)
            else:
                prev_out, cache[layer + 1] = affine_relu_forward(prev_out, W, b)
                
            if self.use_dropout:
                prev_out, dropout_cache[layer + 1] = dropout_forward(prev_out, self.dropout_param)
        
        W = self.params['W' + str(self.num_layers)]
        b = self.params['b' + str(self.num_layers)]
        scores, cache[self.num_layers] = affine_forward(prev_out, W, b)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        # grad compute
        dReLU, dW, db = affine_backward(dS, cache[self.num_layers])
        ...
        for i in reversed(range(self.num_layers - 1)):
            if self.use_dropout:
                dReLU = dropout_backward(dReLU, dropout_cache[i + 1])
            
            if self.use_batchnorm:
                dReLU, dW, db, dgamma, dbeta = batchnorm_relu_backward(dReLU, cache[i + 1])
                grads['gamma' + str(i + 1)] = dgamma
                grads['beta' + str(i + 1)] = dbeta
                dW += self.reg * self.params['W' + str(i + 1)]
            else:
                dReLU, dW, db = affine_relu_backward(dReLU, cache[i + 1])
                dW += self.reg * self.params['W' + str(i + 1)]
